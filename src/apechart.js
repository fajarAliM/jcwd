...
export default class ApexCharts {
  ...
  render() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.el !== null) {
        ...
        const {beforeMount} = this.w.config.chart.events
        ...
        if (!this.css) {
          ...
          const globalCSS = doc.getElementById("apexcharts-css")
  ...
  mount(graphData = null) {
    ...
    return new Promise((resolve, reject) => {
      ...
      } if (graphData === null || w.globals.allSeriesCollapsed) {
      ...
      const yaxis = new YAxis(this.ctx)
      if (elgrid !== null) {
      ...
      if (!w.globals.noData) {
        ...
        if (
          ...
          const {tools} = w.config.chart.toolbar
          const toolsArr = [
            "zoom",
            "zoomin",
            "zoomout",
